...
# Hexa Puffs MCP stack: Orchestrator, Thinker, and all HTTP/stdio MCPs.
#
# Prerequisites:
...
TOKEN_FILE="$HOME/.hexa-puffs/hexa-puffs.token"
if [ -f "$TOKEN_FILE" ]; then
  HEXA_PUFFS_TOKEN=$(cat "$TOKEN_FILE")
  export HEXA_PUFFS_TOKEN
  TOKEN_HEADER=(-H "X-Hexa-Puffs-Token: $HEXA_PUFFS_TOKEN")
else
  HEXA_PUFFS_TOKEN=""
  TOKEN_HEADER=()
fi
...
if [ "$RUN_VITEST" = true ]; then
  ...
  # =====================================================
  # Section 4: Integration Tests (Stdio transport)
  # =====================================================
  # Now running via Stdio transport using MCPStdioTestClient
  print_section "Section 4: Integration Tests (Stdio)"

  # Searcher (npm run test:web)
  echo -e "  Running ${BOLD}Searcher${RESET} integration tests..."
  cd "$MCP_DIR/Searcher-MCP"
  if npm run test:web 2>&1; then
    echo -e "  ${GREEN}✓ Searcher tests passed${RESET}"
    ((PASSED++)) || true
  else
    echo -e "  ${RED}✗ Searcher tests failed${RESET}"
    ((FAILED++)) || true
  fi
  cd "$MCP_DIR"

  # Filer (npm run test:filer)
  echo -e "  Running ${BOLD}Filer${RESET} integration tests..."
  cd "$MCP_DIR/Filer-MCP"
  if npm run test:filer 2>&1; then
    echo -e "  ${GREEN}✓ Filer tests passed${RESET}"
    ((PASSED++)) || true
  else
    echo -e "  ${RED}✗ Filer tests failed${RESET}"
    ((FAILED++)) || true
  fi
  cd "$MCP_DIR"

  # Telegram (npm run test:integration)
  echo -e "  Running ${BOLD}Telegram${RESET} integration tests..."
  cd "$MCP_DIR/Telegram-MCP"
  if npm run test:integration 2>&1; then
    echo -e "  ${GREEN}✓ Telegram tests passed${RESET}"
    ((PASSED++)) || true
  else
    echo -e "  ${RED}✗ Telegram tests failed${RESET}"
    ((FAILED++)) || true
  fi
  cd "$MCP_DIR"
