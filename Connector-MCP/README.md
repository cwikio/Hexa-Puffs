# Hexa Puffs Connector MCP

This component is an **MCP Server** that acts as a bridge between **MCP Clients** (like Claude Desktop) and the **Hexa Puffs Orchestrator**.

## Purpose

Standard MCPs in this repository (like `Gmail-MCP`) provide tools *to* Hexa Puffs.
This Connector works in reverse: it exposes Hexa Puffs's capabilities *as tools* to an MCP Client.

**Architecture:**
`Client (e.g. Claude)` <-> `Connector-MCP (stdio)` <-> `Orchestrator (HTTP)` <-> `Hexa Puffs Agent`

## Exposed Tools

1.  **`chat`**: Sends a message to Hexa Puffs and waits for her response.
...
**Prerequisite:** Ensure Hexa Puffs Orchestrator is running (`npm start` in `Orchestrator` directory).

**Configuration:**
Add the `Hexa Puffs` entry to your `mcpServers` list.
...
*   **Fix**:
    *   If you started Hexa Puffs via `./restart.sh`, use `http://localhost:8010`.
    *   If you started Hexa Puffs manually (`npm start` in `Orchestrator`), use `http://localhost:8000`.
