# Hexa Puffs Orchestrator MCP

The central orchestration layer for Hexa Puffs AI Assistant. The Orchestrator acts as an **agent router and protocol bridge** that:
...
2. **Add an `hexa-puffs` manifest** to your `package.json`:
...
2. Skips directories without an `hexa-puffs.mcpName` field (Orchestrator, Shared, Thinker, etc.)
...
- **Persistent state:** Halt state is saved to `~/.hexa-puffs/data/halt.json` — survives Orchestrator restarts
...
```bash
docker build -t hexa-puffs-orchestrator .
docker run -p 8010:8010 hexa-puffs-orchestrator
```
...
Returns `{ "success": true, "message": "Agent \"hexa-puffs\" resumed" }` on success.
