...
const DATA_DIR = join(homedir(), '.hexa-puffs', 'data');
const DOCS_DIR = join(homedir(), '.hexa-puffs', 'documentation');
...
async function checkFailedTasks(_ctx: DiagnosticContext): Promise<DiagnosticFinding | null> {
  try {
    ...
    return {
      ...
      recommendation: 'Check task details in ~/.hexa-puffs/data/tasks/',
    };
  } catch {
    return null;
  }
