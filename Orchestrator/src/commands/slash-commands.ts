...
const LOGS_DIR = join(homedir(), '.hexa-puffs', 'logs')
...
const SERVICE_LOG_FILES = ['orchestrator.log', 'gmail.log', 'telegram.log', 'searcher.log', 'filer.log', 'memorizer.log', 'ollama.log', 'web.log']
...
export class SlashCommandHandler {
  ...
  private async handleStatus(args: string): Promise<string> {
    ...
    if (webMcp) {
      if (webMcp.available) {
        ...
        try {
          const tabResult = await Promise.race([this.toolRouter.routeToolCall('web_browser_tabs', { action: 'list' }), new Promise<null>((resolve) => setTimeout(() => resolve(null), 3000))])
          if (tabResult) {
  ...
  private async handleStatusSummary(): Promise<string> {
    ...
    const [statusData, logIssues, securityResult, memoryStatsResult, fileAuditResult, cronJobs, cronSkillsResult, backgroundTasks, inngestServerUp, inngestEndpointUp] = await Promise.all([Promise.resolve(this.orchestrator.getStatus()), this.parseRecentLogIssues(), this.orchestrator.callGuardianTool('get_scan_log', { limit: 20, threats_only: true }).catch(() => null), this.toolRouter.routeToolCall('memory_get_memory_stats', { agent_id: 'hexa-puffs' }).catch(() => null), this.toolRouter.routeToolCall('filer_get_audit_log', { limit: 20 }).catch(() => null), new JobStorage().listJobs().catch(() => []), this.toolRouter.routeToolCall('memory_list_skills', { agent_id: 'thinker', enabled: true, trigger_type: 'cron' }).catch(() => null), new JobStorage().listTasks().catch(() => []), this.checkHealth(summaryJobsConfig.inngestUrl), this.checkHealth(`http://localhost:${summaryJobsConfig.port}/health`)])
    ...
    if (backgroundTasks.length === 0) {
      ...
      const recentTasks = backgroundTasks.sort((a: TaskDefinition, b: TaskDefinition) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 10)
...
Keep it concise. No markdown formatting — plain text only.`
  ...
  private async handleCron(): Promise<string> {
    ...
    const [serverUp, endpointUp, cronJobs, tasks, skillsResult] = await Promise.all([
      ...
      this.toolRouter
        .routeToolCall('memory_list_skills', {
          agent_id: 'thinker',
          enabled: true,
          trigger_type: 'cron',
        })
    ...
    const jobsSummary = [enabledCount > 0 ? `${enabledCount} enabled` : null, disabledCount > 0 ? `${disabledCount} disabled` : null].filter(Boolean).join(', ')
    ...
    if (skills.length === 0) {
      ...
      for (const skill of skills) {
        ...
        const status = skill.lastStatus === 'failed' || skill.lastStatus === 'error' ? 'error' : (skill.lastStatus ?? '-')
        output += `  ${id} | ${name} | ${schedule} | ${lastRun} | ${status}\n`
      }
    }

    // --- Tasks ---
    const sorted = tasks.sort((a: TaskDefinition, b: TaskDefinition) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 10)
    ...
    const taskParts = [running > 0 ? `${running} running` : null, queued > 0 ? `${queued} queued` : null, completed > 0 ? `${completed} completed` : null, failed > 0 ? `${failed} failed` : null].filter(Boolean).join(', ')
  ...
  private async handleLinkAll(args: string): Promise<string> {
    ...
    for (const mcpName of externalMCPNames) {
      ...
      const discovery = config.mcpServersStdio?.[mcpName]?.metadata?.projectDiscovery ?? config.mcpServersHttp?.[mcpName]?.metadata?.projectDiscovery
  ...
  private async handleScanProjects(): Promise<string> {
    try {
      ...
      return 'Full project scan triggered. Please wait...'
    } catch (error) {
  ...
  private async handleProjectStatus(args: string): Promise<string> {
    ...
    if (sources.length === 0) {
      return query ? `No linked sources found for "${query}".` : 'No linked project sources found. Use /link to connect projects to MCPs.'
    }
  ...
  private async handleSetupMonitor(args: string): Promise<string> {
    ...
    const requiredTools = ['memory_list_project_sources', 'memory_update_project_source_status', 'telegram_send_message']
    ...
    const humanInterval = intervalMinutes >= 60 ? `${Math.floor(intervalMinutes / 60)}h${intervalMinutes % 60 > 0 ? ` ${intervalMinutes % 60}m` : ''}` : `${intervalMinutes}m`
   ...
   */
  private extractProjectItems(data: unknown, idField: string, nameField: string): Array<{ id: unknown; name: unknown }> {
    if (Array.isArray(data)) {
      return data.filter((item) => item && typeof item === 'object').map((item) => ({ id: (item as Record<string, unknown>)[idField], name: (item as Record<string, unknown>)[nameField] }))
    }
  ...
  private parseDeleteArgs(args: string): { type: 'today' } | { type: 'yesterday' } | { type: 'week' } | { type: 'hours'; value: number } | { type: 'count'; value: number } | { type: 'invalid'; reason: string } {
    const trimmed = args.trim().toLowerCase()
  ...
  private async handleLogStatus(): Promise<string> {
    ...
    try {
      ...
      return 'Cannot read log directory: ~/.hexa-puffs/logs/'
    }

    if (files.length === 0) return 'No log files found.'

    let output = 'System Logs (~/.hexa-puffs/logs/)\n\n'
    let totalSize = 0
