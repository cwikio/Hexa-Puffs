...
import { handleListTools, handleCallTool, handleChat } from './core/http-handlers.js';
...
const HEXA_PUFFS_TOKEN = process.env.HEXA_PUFFS_TOKEN;

async function main(): Promise<void> {
  const config = getConfig();

  if (config.transport !== 'stdio' && !HEXA_PUFFS_TOKEN) {
    logger.warn('HEXA_PUFFS_TOKEN is not set — HTTP endpoints are unauthenticated. Run via start-all.sh for automatic token generation.');
  }

  logger.info('Starting Hexa Puffs Orchestrator', {
  ...
  try {
    ...
    // Always start HTTP server for REST API (Tools, Chat, Status)
    const httpServer = createHttpServer(async (req, res) => {
      ...
      res.setHeader('Access-Control-Allow-Headers', 'Content-Type, X-Hexa-Puffs-Token');
      ...
      if (HEXA_PUFFS_TOKEN && req.headers['x-hexa-puffs-token'] !== HEXA_PUFFS_TOKEN) {
      ...
      // REST API: Data Channel (Chat)
      if (req.url === '/chat' && req.method === 'POST') {
          await handleChat(orchestrator, req, res);
          return;
      }
      ...
      // SSE endpoint (only if requested, acts as a transport)
    ...
    await new Promise<void>((resolve) => {
      httpServer.listen(config.port, '127.0.0.1', () => {
        logger.info(`Orchestrator HTTP API running on http://localhost:${config.port}`);
        resolve();
      });
    });
    ...
    if (config.transport === 'stdio') {
      ...
      logger.info('Orchestrator running on stdio transport');
       ...
       // Typically SSE is handled via the HTTP server we just started
       logger.info('Orchestrator HTTP/SSE transport ready');
