...
describe('E2E: Skill Tier Routing & Verification', () => {
  ...
  beforeAll(async () => {
    ...
    // Clean up stale E2E test skills from previous crashed runs
    const allSkills = await callToolRaw('memory_list_skills', { agent_id: agentId });
    const skills = ((allSkills as Record<string, unknown>).data as Record<string, unknown>)?.skills;
    if (Array.isArray(skills)) {
      for (const skill of skills) {
        if (typeof skill.name === 'string' && skill.name.startsWith('E2E ')) {
          try {
            await callToolRaw('memory_delete_skill', { skill_id: skill.id });
            log(`Cleaned stale E2E skill: ${skill.name} (id=${skill.id})`, 'debug');
          } catch { /* ignore */ }
        }
      }
    }
