...
import { hasValidToken } from "./outlook/auth.js";
...
async function main(): Promise<void> {
  ...
  logger.info("Starting Outlook MCP", {
  ...
  if (!hasValidToken()) {
    logger.error(
      "No Outlook token cache found. MCP will start in degraded mode — all tool calls will fail. Run 'npm run setup-oauth' to authenticate."
    );
  }
  ...
  await startTransport(server, {
    ...
    onHealth: () => {
      ...
      return {
        ...
        hasToken: tokenValid,
        ...(tokenValid ? {} : { message: "No Outlook token. Run 'npm run setup-oauth' to authenticate." }),
    ...
    onToolCall: async (name: string, args: unknown) => {
      ...
      if (!hasValidToken()) {
        throw new Error(
          "Outlook token missing or invalid. Run 'npm run setup-oauth' in the Outlook-MCP directory to authenticate."
        );
      }
      return handler(args);
    },
