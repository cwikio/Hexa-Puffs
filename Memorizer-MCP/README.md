# Hexa Puffs Memory MCP

Persistent memory for the Hexa Puffs AI Assistant. Learns from every interaction, recalls relevant context, and builds a rich user profile over time.
...
```bash
MEMORY_DB_PATH=~/.hexa-puffs/data/memory.db
MEMORY_EXPORT_PATH=~/.hexa-puffs/memory-export/
```
...
Exported to `~/.hexa-puffs/memory-export/`:
