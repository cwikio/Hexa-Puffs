...
# Launch script for Hexa Puffs MCP Stack
#
# MCPs are auto-discovered from package.json "hexa-puffs" manifests.
...
echo -e "${BOLD}${BLUE}=== Launching Hexa Puffs MCP Stack ===${RESET}\n"
...
if [ -z "$DISCOVERY" ]; then
  echo -e "${RED}No MCPs discovered! Check package.json hexa-puffs manifests.${RESET}"
  exit 1
fi
...
PID_FILE="$HOME/.hexa-puffs/hexa-puffs.pids"
: > "$PID_FILE"

# ─── Auth Token ──────────────────────────────────────────────────────────────
HEXA_PUFFS_TOKEN=$(openssl rand -hex 32)
echo "$HEXA_PUFFS_TOKEN" > "$HOME/.hexa-puffs/hexa-puffs.token"
chmod 600 "$HOME/.hexa-puffs/hexa-puffs.token"
export HEXA_PUFFS_TOKEN
echo -e "${GREEN}✓ Auth token generated and saved to ~/.hexa-puffs/hexa-puffs.token${RESET}"

# ─── Persona & Skills Initialization ──────────────────────────────────────────
echo -e "${BOLD}${CYAN}=== Initializing Persona & Skills ===${RESET}"

AGENTS_DIR="$HOME/.hexa-puffs/agents"
SKILLS_DIR="$HOME/.hexa-puffs/skills"
...
HEXA_PUFFS_PERSONA_DIR="$AGENTS_DIR/hexa-puffs"
HEXA_PUFFS_PERSONA_FILE="$HEXA_PUFFS_PERSONA_DIR/instructions.md"
DEFAULT_PERSONA_SRC="$SCRIPT_DIR/Thinker/defaults/personas/hexa-puffs/instructions.md"
if [ ! -f "$HEXA_PUFFS_PERSONA_FILE" ]; then
  if [ -f "$DEFAULT_PERSONA_SRC" ]; then
    mkdir -p "$HEXA_PUFFS_PERSONA_DIR"
    cp "$DEFAULT_PERSONA_SRC" "$HEXA_PUFFS_PERSONA_FILE"
    echo -e "  ${GREEN}✓ Copied default Hexa Puffs persona to $HEXA_PUFFS_PERSONA_FILE${RESET}"
  ...
  echo -e "  ${BLUE}✓ Hexa Puffs persona already exists${RESET}"
fi
...
if ! kill -0 "$INNGEST_PID" 2>/dev/null; then
  echo -e "${YELLOW}⚠ Inngest Dev Server process died — check ~/.hexa-puffs/logs/inngest.log${RESET}"
fi
...
if ! command -v ollama &> /dev/null; then
  ...
  ollama serve >> ~/.hexa-puffs/logs/ollama.log 2>&1 &
  OLLAMA_PID=$!
  for i in {1..10}; do
...
TRANSPORT=http PORT=8010 MCP_CONNECTION_MODE=stdio \
  ...
  HEXA_PUFFS_TOKEN="$HEXA_PUFFS_TOKEN" \
...
if ! kill -0 "$ORCHESTRATOR_PID" 2>/dev/null; then
  echo -e "${RED}${BOLD}✗ Orchestrator process died — aborting. Check ~/.hexa-puffs/logs/orchestrator.log${RESET}"
  exit 1
fi
...
if echo "$ORCHESTRATOR_HEALTH" | grep -q "ok"; then
  ...
  TOOLS_RESPONSE=$(curl -s -H "X-Hexa-Puffs-Token: $HEXA_PUFFS_TOKEN" http://localhost:8010/tools/list 2>/dev/null)
  ...
  echo -e "  ${YELLOW}Check logs: tail -f ~/.hexa-puffs/logs/orchestrator.log${RESET}"
fi
...
if [ -f "$SEED_SCRIPT" ]; then
  ORCHESTRATOR_URL=http://localhost:8010 npx tsx "$SEED_SCRIPT" >> ~/.hexa-puffs/logs/seed-skills.log 2>&1 &
  SEED_PID=$!
  echo -e "${GREEN}✓ Skill seeding started in background (PID: $SEED_PID)${RESET}"
  echo -e "  ${BLUE}Check progress: cat ~/.hexa-puffs/logs/seed-skills.log${RESET}"
else
...
if [ -f "$SNAPSHOT_SCRIPT" ]; then
  ORCHESTRATOR_URL=http://localhost:8010 HEXA_PUFFS_TOKEN="$HEXA_PUFFS_TOKEN" \
    npx tsx "$SNAPSHOT_SCRIPT" >> ~/.hexa-puffs/logs/snapshot.log 2>&1 &
  SNAPSHOT_PID=$!
  echo -e "${GREEN}✓ Snapshot generation started in background (PID: $SNAPSHOT_PID)${RESET}"
  echo -e "  ${BLUE}Output: ~/.hexa-puffs/documentation/system-snapshot.md${RESET}"
else
...
echo -e "    └── Thinker :8006 (hexa-puffs) — cost controls enabled"
...
echo -e "  $(printf '%-14s' "Orchestrator:") ~/.hexa-puffs/logs/orchestrator.log"
echo -e "  $(printf '%-14s' "Thinker:") (inside orchestrator.log — grep '[thinker:hexa-puffs]')"
echo -e "  $(printf '%-14s' "Inngest:") ~/.hexa-puffs/logs/inngest.log"
...
if [ -d "$CONNECTOR_DIR" ]; then
  ...
  echo -e "  To connect Claude Desktop to Hexa Puffs, add this to your ${BOLD}'mcpServers'${RESET} config:"
  echo -e "${BLUE}"
  echo -e "    \"Hexa Puffs\": {"
  echo -e "      \"command\": \"node\","
  echo -e "      \"args\": ["
