...
Shared types, utilities, and abstractions used across all Hexa Puffs MCP packages.
...
| File | Description |
|---|---|
| `scanner.ts` | `scanForMCPs()` — reads sibling directories, extracts `"hexa-puffs"` field from `package.json` |
| `types.ts` | `Hexa PuffsManifest`, `DiscoveredMCP`, `ChannelManifestConfig` type definitions |
