...
/** Read the Hexa Puffs auth token from env or ~/.hexa-puffs/hexa-puffs.token */
export function resolveToken(explicit?: string): string | undefined {
  if (explicit) return explicit;
  if (process.env.HEXA_PUFFS_TOKEN) return process.env.HEXA_PUFFS_TOKEN;
  try {
    return readFileSync(join(homedir(), '.hexa-puffs', 'hexa-puffs.token'), 'utf-8').trim();
  } catch {
    return undefined;
  }
}
...
export class MCPTestClient {
  ...
  private authHeaders(): Record<string, string> {
    if (this.token) return { 'X-Hexa-Puffs-Token': this.token };
    return {};
  }
