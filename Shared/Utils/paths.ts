...
export class PathManager {
  ...
  private constructor() {
    this.homeDir = process.env.HEXA_PUFFS_HOME 
      ? resolve(process.env.HEXA_PUFFS_HOME)
      : join(homedir(), '.hexa-puffs');
  }
  ...
  /**
   * Resolve a path relative to HEXA_PUFFS_HOME if it starts with ~/.hexa-puffs or just ~
   */
  public resolvePath(path: string): string {
    if (path.startsWith(this.homeDir)) {
      return path;
    }
    if (path.startsWith('~/.hexa-puffs')) {
      return path.replace('~/.hexa-puffs', this.homeDir);
    }
    if (path.startsWith('~/')) {
