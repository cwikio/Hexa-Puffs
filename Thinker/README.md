...
│  │  ├─ Logs all events to ~/.hexa-puffs/logs/traces.jsonl             │   │
...
EMBEDDING_CACHE_DIR=~/.hexa-puffs/data    # Directory for embedding cache file

# Logging
LOG_LEVEL=info
TRACE_LOG_PATH=~/.hexa-puffs/logs/traces.jsonl
```
...
2. Embeddings are cached to `~/.hexa-puffs/data/embedding-cache.json` — subsequent startups skip re-embedding unchanged tools
...
| `EMBEDDING_CACHE_DIR` | `~/.hexa-puffs/data` | Directory for cache file |
...
**User can edit via:**
- `~/.hexa-puffs/memory-export/profiles/thinker.md`
...
│   │
│   ├── session/
│   │   ├── store.ts             # Session file persistence (~/.hexa-puffs/sessions/)
...
- [ ] JSONL logger to `~/.hexa-puffs/logs/traces.jsonl`
...
6. **Check trace log:**
   ...
   ```bash
   tail -f ~/.hexa-puffs/logs/traces.jsonl
   ```
