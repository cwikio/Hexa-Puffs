...
/**
 * Ensure the Thinker agent is running by triggering the Orchestrator's
 * lazy-spawn (via /chat) and polling until the Thinker health endpoint responds.
 *
 * The Orchestrator lazy-spawns Thinker on first message dispatch, so this
 * sends a lightweight message to trigger the spawn, then waits for readiness.
 */
export async function ensureThinkerRunning(
  maxWaitMs: number = 30_000
): Promise<boolean> {
  // Already running?
  const quick = await checkThinkerHealth()
  if (quick) return true

  log('Thinker not running — triggering lazy-spawn via Orchestrator /chat', 'info')
  ...
  try {
    ...
    token =
      process.env.HEXA_PUFFS_TOKEN ||
      readFileSync(join(homedir(), '.hexa-puffs', 'hexa-puffs.token'), 'utf-8').trim()
  } catch {
    token = undefined
  }

  const headers: Record<string, string> = { 'Content-Type': 'application/json' }
  if (token) headers['X-Hexa-Puffs-Token'] = token

  // Trigger the Orchestrator to dispatch a message — this calls ensureRunning()
  try {
    await fetch(`${ORCHESTRATOR_URL}/chat`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ message: 'ping', sender: 'test-warmup' }),
      signal: AbortSignal.timeout(15_000),
    })
  } catch {
    log('Failed to trigger Orchestrator /chat — Thinker may not start', 'warn')
  }

  // Poll Thinker health until it responds
  const deadline = Date.now() + maxWaitMs
  while (Date.now() < deadline) {
    await new Promise((r) => setTimeout(r, 1_000))
    if (await checkThinkerHealth()) {
      log('Thinker is now running', 'success')
      return true
    }
  }

  log('Thinker did not become available within timeout', 'warn')
  return false
}

async function checkThinkerHealth(): Promise<boolean> {
  try {
    const res = await fetch(`${THINKER_URL}/health`, {
...
export async function checkTraceLogExists(
  logPath: string = '~/.hexa-puffs/logs/traces.jsonl'
): Promise<boolean> {
