...
describe('Proactive Tasks (/execute-skill)', () => {
  ...
  beforeAll(async () => {
    ...
    // Check Orchestrator availability (needed for tool routing + lazy-spawn)
    ...
    if (!orchestratorAvailable) {
      ...
      // Thinker is lazy-spawned by Orchestrator — trigger startup and wait
      thinkerAvailable = await ensureThinkerRunning()
      if (!thinkerAvailable) {
  ...
  describe('Skill Execution', () => {
    it('should execute a simple instruction and return structured result', async () => {
      ...
      if (data.paused || data.summary?.includes('paused by cost controls')) {
