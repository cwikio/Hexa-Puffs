...
Generated: 2026-03-04T16:01:52.482Z
...
|---|---------|----------|-----------|---------|---------|-------|
| 1 | search for AI news | `searcher_news_search` | yes | 12 | 13 | 0 |
| 2 | send an email to bob about the project update | `memory_list_contacts` | no | 25 | 16 | -9 |
| 3 | what meetings do I have tomorrow | `gmail_list_events` | yes | 16 | 15 | -1 |
| 4 | read the file report.txt | `filer_read_file` | yes | 20 | 15 | -5 |
| 5 | remember I like dark mode | `memory_store_fact` | no | 25 | 16 | -9 |
| 6 | show me pictures of cats | `searcher_image_search` | yes | 25 | 15 | -10 |
| 7 | what do you know about me | `memory_get_profile` | yes | 25 | 15 | -10 |
| 8 | check my password vault for github | `onepassword_list_vaults` | yes | 12 | 13 | 0 |
| 9 | navigate to google.com and take a screenshot | `web_browser_navigate` | yes | 16 | 15 | -1 |
| 10 | run the python script | `codexec_list_scripts` | yes | 19 | 15 | -4 |
| 11 | delete the spam email from my inbox | `gmail_list_emails` | yes | 25 | 15 | -10 |
| 12 | create a new calendar event for Friday at 3pm | `gmail_quick_add_event` | yes | 14 | 14 | 0 |
| 13 | send a message to the family group on telegram | `telegram_list_chats` | yes | 25 | 15 | -10 |
| 14 | what is the weather in Warsaw | `searcher_web_search` | yes | 13 | 14 | 0 |
| 15 | save this note to my workspace | _null_ | - | 20 | 20 | 0 |
| 16 | hello how are you | _null_ | - | 25 | 25 | 0 |
| 17 | remind me every morning to check email | `memory_store_skill` | no | 25 | 16 | -9 |
| 18 | find a picture of sunset and send it to the group | `searcher_image_search` | yes | 25 | 15 | -10 |
| 19 | who is the president of France | _null_ | - | 12 | 12 | 0 |
| 20 | browse to amazon.com and search for headphones | `web_browser_navigate` | yes | 20 | 15 | -5 |

**LLM picks:** 17/20 (85%)
**Average tools — default:** 19.9, **reduced:** 15.4, **reduction:** 22.6%
...
- `gmail_reply_email` (0.539)
- `gmail_create_draft` (0.537)
- `gmail_list_drafts` (0.515)
- `gmail_list_emails` (0.507)

**Dropped vs default (10):** ~~`gmail_delete_draft`~~, ~~`gmail_delete_email`~~, ~~`gmail_get_attachment`~~, ~~`gmail_get_email`~~, ~~`gmail_list_attachments`~~, ~~`gmail_mark_read`~~, ~~`gmail_modify_labels`~~, ~~`search_memories`~~, ~~`spawn_subagent`~~, ~~`store_fact`~~
...
**Core tools (5):** `get_status`, `search_memories`, `searcher_web_search`, `send_telegram`, `spawn_subagent`
...
- `gmail_delete_event` (0.554)
- `gmail_find_free_time` (0.546)

**Dropped vs default (1):** ~~`store_fact`~~
...
- `filer_create_file` (0.546)
- `filer_move_file` (0.544)
- `filer_request_grant` (0.542)
- `filer_get_workspace_info` (0.537)

**Dropped vs default (5):** ~~`filer_list_grants`~~, ~~`get_status`~~, ~~`search_memories`~~, ~~`spawn_subagent`~~, ~~`store_fact`~~
...
- `memory_query_timeline` (0.501)
- `memory_get_skill` (0.499)
- `memory_search_conversations` (0.489)

**Dropped vs default (9):** ~~`get_status`~~, ~~`memory_delete_skill`~~, ~~`memory_list_projects`~~, ~~`memory_store_conversation`~~, ~~`memory_store_skill`~~, ~~`memory_update_fact`~~, ~~`memory_update_project`~~, ~~`spawn_subagent`~~, ~~`store_fact`~~
...
- `telegram_get_messages` (0.470)
- `telegram_list_chats` (0.469)
- `searcher_news_search` (0.464)

**Dropped vs default (11):** ~~`get_status`~~, ~~`spawn_subagent`~~, ~~`telegram_add_contact`~~, ~~`telegram_get_me`~~, ~~`telegram_get_new_messages`~~, ~~`telegram_list_contacts`~~, ~~`telegram_mark_read`~~, ~~`telegram_search_messages`~~, ~~`telegram_search_users`~~, ~~`telegram_send_message`~~, ~~`telegram_subscribe_chat`~~
...
**Core tools (4):** `get_status`, `searcher_web_search`, `send_telegram`, `store_fact`
...
- `memory_list_projects` (0.474)
- `memory_search_conversations` (0.473)
- `memory_create_contact` (0.471)

**Dropped vs default (10):** ~~`gmail_get_email`~~, ~~`memory_export_memory`~~, ~~`memory_get_skill`~~, ~~`memory_list_facts`~~, ~~`memory_list_skills`~~, ~~`memory_update_contact`~~, ~~`memory_update_profile`~~, ~~`memory_update_project_source_status`~~, ~~`search_memories`~~, ~~`spawn_subagent`~~
...
**Core tools (5):** `get_status`, `search_memories`, `searcher_web_search`, `send_telegram`, `store_fact`
...
- `web_browser_close` (0.512)
- `searcher_news_search` (0.503)

**Dropped vs default (1):** ~~`spawn_subagent`~~
...
**Core tools (4):** `get_status`, `searcher_web_search`, `send_telegram`, `spawn_subagent`
...
- `codexec_start_session` (0.456)
- `codexec_close_session` (0.395)

**Dropped vs default (4):** ~~`codexec_list_sessions`~~, ~~`codexec_send_to_session`~~, ~~`search_memories`~~, ~~`store_fact`~~
...
- `gmail_reply_email` (0.596)
- `gmail_send_email` (0.586)
- `gmail_update_draft` (0.579)
- `gmail_list_attachments` (0.577)

**Dropped vs default (11):** ~~`get_status`~~, ~~`gmail_create_draft`~~, ~~`gmail_create_filter`~~, ~~`gmail_get_attachment`~~, ~~`gmail_list_drafts`~~, ~~`gmail_list_filters`~~, ~~`gmail_mark_read`~~, ~~`gmail_send_draft`~~, ~~`search_memories`~~, ~~`spawn_subagent`~~, ~~`store_fact`~~
...
- **LLM Pick (4B):** `telegram_list_chats` — confirmed by embeddings
- **Total tools sent to Groq:** 15
...
- `telegram_get_new_messages` (0.637)
- `telegram_subscribe_chat` (0.628)
- `telegram_download_media` (0.617)

**Dropped vs default (10):** ~~`get_status`~~, ~~`gmail_reply_email`~~, ~~`gmail_send_draft`~~, ~~`gmail_send_email`~~, ~~`search_memories`~~, ~~`spawn_subagent`~~, ~~`store_fact`~~, ~~`telegram_get_chat`~~, ~~`telegram_get_me`~~, ~~`telegram_list_contacts`~~
...
**Core tools (5):** `get_status`, `search_memories`, `send_telegram`, `spawn_subagent`, `store_fact`
...
- `memory_update_project_source_status` (0.437)
- `memory_search_conversations` (0.436)
- `memory_retrieve_memories` (0.436)
- `memory_get_skill` (0.433)
- `memory_update_skill` (0.432)
...
- `gmail_delete_email` (0.586)
- `gmail_create_draft` (0.579)
- `gmail_create_filter` (0.567)
- `gmail_modify_labels` (0.552)

**Dropped vs default (10):** ~~`get_status`~~, ~~`gmail_delete_draft`~~, ~~`gmail_delete_filter`~~, ~~`gmail_get_attachment`~~, ~~`gmail_get_filter`~~, ~~`gmail_list_filters`~~, ~~`gmail_send_draft`~~, ~~`search_memories`~~, ~~`spawn_subagent`~~, ~~`store_fact`~~
...
- `searcher_news_search` (0.504)
- `telegram_subscribe_chat` (0.499)
- `gmail_send_email` (0.493)
- `telegram_search_messages` (0.492)

**Dropped vs default (10):** ~~`get_status`~~, ~~`gmail_create_draft`~~, ~~`gmail_reply_email`~~, ~~`spawn_subagent`~~, ~~`store_fact`~~, ~~`telegram_add_contact`~~, ~~`telegram_get_chat`~~, ~~`telegram_get_new_messages`~~, ~~`telegram_mark_read`~~, ~~`telegram_search_users`~~
...
- `searcher_news_search` (0.479)
- `searcher_image_search` (0.466)
- `web_browser_click` (0.465)
- `web_browser_close` (0.464)

**Dropped vs default (5):** ~~`spawn_subagent`~~, ~~`web_browser_navigate_back`~~, ~~`web_browser_snapshot`~~, ~~`web_browser_take_screenshot`~~, ~~`web_browser_type`~~
