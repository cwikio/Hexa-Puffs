...
An MCP (Model Context Protocol) server that provides full Gmail access to AI assistants. Part of the Hexa Puffs AI assistant system, it allows Thinker to check, send, and manage emails via Telegram commands.
...
8. Download the JSON file and save it to `~/.hexa-puffs/gmail/credentials.json`
...
This opens your browser to Google's consent screen. After granting access, the refresh token is saved to `~/.hexa-puffs/gmail/token.json`. You only need to do this once.
...
### As part of the Hexa Puffs system
...
| `GMAIL_CREDENTIALS_PATH` | `~/.hexa-puffs/gmail/credentials.json` | Path to OAuth credentials JSON |
| `GMAIL_TOKEN_PATH` | `~/.hexa-puffs/gmail/token.json` | Path to stored OAuth token |
