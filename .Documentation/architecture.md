...
All stdio MCPs are **child processes** spawned by the Orchestrator. They communicate via stdin/stdout using the MCP JSON-RPC protocol. The Orchestrator is the only process that talks to all MCPs.
...
- `create_job`, `list_jobs`, `get_job_status`, `delete_job` — Inngest jobs
