...
**Post-dispatch** (`orchestrator.ts:452-496`): When Thinker returns `paused: true`:
...
**Pre-dispatch** (`orchestrator.ts:433-439`): On subsequent messages:
...
```
Agent "hexa-puffs" has been paused due to unusual token consumption.
...
- Returns: `{ "success": true, "message": "Agent \"hexa-puffs\" resumed" }`
...
Source: `Thinker/src/index.ts:216-238`
...
| Thinker HTTP endpoints | `Thinker/src/index.ts` | 216–238 |
...
| Dispatch interception | `Orchestrator/src/core/orchestrator.ts` | 433–439, 452–496 |
