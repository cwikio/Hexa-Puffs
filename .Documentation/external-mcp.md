...
This document describes how third-party MCP servers are integrated into the Hexa Puffs ecosystem without modifying the core codebase. It covers the config format, loading, health monitoring, startup awareness, hot-reload, and the Thinker's system prompt integration.
...
8. [Thinker Awareness](#thinker-awareness)
...
Internal MCPs (Guardian, Filer, Memorizer, etc.) are auto-discovered from sibling directories via the `"hexa-puffs"` manifest in `package.json`. External MCPs are third-party servers declared in a single config file — `external-mcps.json` — in the project root. They are loaded alongside internal MCPs at startup and treated identically by the ToolRouter once connected.

Key properties of external MCPs:
- Never `required` — a failed external MCP does not block Orchestrator startup
- Scanned by Guardian by default (`defaultInput: true`, `defaultOutput: true`) — override via `metadata.guardianScan`
...
│  └────┬─────┘ └────┬─────┘ └────┬─────┘     └──────┬───────┘   │
│       │             │            │                   │           │
│       │  Auto-discovered via package.json "hexa-puffs" manifest  │
│       └──────────┬──┘────────────┘───────────────────┘           │
│                  │                                               │
│                  ▼                                               │
│  ┌──────────────────────────────────────────────────────────┐   │
...
| `sensitive` | boolean | No | `false` | If true, adds MCP prefix to `sensitiveTools` list (separate from Guardian scanning) |
| `description` | string | No | — | Human-readable description (shown in startup notification) |
| `metadata` | object | No | — | Optional metadata: `label`, `toolGroup`, `keywords`, `guardianScan` (`{ input, output }`), `allowDestructiveTools` |
...
- Any valid identifier is matched (e.g., `${MY_KEY}`, `${myKey}`) — the regex accepts `[A-Za-z_][A-Za-z0-9_]*`
- Env var placeholders in `args` array values are also resolved (not just `env`)
...
The notification is sent to the chat ID configured for the `hexa-puffs` agent in `agents.json`:
...
The Thinker agent learns about external MCPs through two mechanisms:

1. **Tool discovery** — External MCP tools appear in the Thinker's tool list after hot-reload (10-minute cache TTL). The LLM can call them like any other tool.
2. **`system_health_check` tool** — Reports per-MCP health with internal/external classification, so the agent can diagnose connectivity issues.

The default system prompt (`Thinker/prompts/default-system-prompt.md`) does not contain explicit sections about external MCPs. Awareness comes from tool availability and the health check tool's output.
...
3. Orchestrator.handleExternalMCPChange() runs:
   ...
   d. Saves snapshot to ~/.hexa-puffs/last-known-mcps.json
...
| Health check tool | `Orchestrator/src/tools/health-check.ts` | Internal/external classification for Thinker awareness |
| Snapshot file | `~/.hexa-puffs/last-known-mcps.json` | Persisted MCP snapshot between boots |
