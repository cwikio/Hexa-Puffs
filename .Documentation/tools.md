...
This document describes how tools are registered, discovered, selected, executed, and optimized across the Hexa Puffs MCP ecosystem.
...
At startup, the Orchestrator's scanner reads sibling directories looking for packages with an `"hexa-puffs"` manifest in their `package.json`:
...
For full details on skill creation, schema, trigger configuration, scheduling, and the two-tier execution model, see [architecture.md](architecture.md).
...
- **Session storage**: JSONL files in `~/.hexa-puffs/sessions/{agentId}/`
