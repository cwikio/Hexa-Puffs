...
1. **Cost controls triggered** — Token consumption exceeded threshold.
   ...
   - Check: `/status` — shows pause reason. Also check `~/.hexa-puffs/logs/thinker.log` for cost monitor messages.
...
3. **LLM provider rate limit** — Groq or other provider returned rate limit error.
   - Check: `~/.hexa-puffs/logs/thinker.log` for rate limit errors.
...
1. **LLM provider latency** — Groq, LM Studio, or Ollama is slow.
   - Check: `~/.hexa-puffs/logs/thinker.log` for LLM call durations.
