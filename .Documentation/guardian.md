...
Guardian is the security layer of the Hexa Puffs MCP ecosystem. It operates as a **transparent decorator** inside the Orchestrator — wrapping downstream MCP clients so that tool inputs and outputs are scanned for prompt injection, social engineering, and other threats before they reach (or leave) a target MCP.
