#!/usr/bin/env npx tsx
/**
 * Generate System Snapshot — writes ~/.hexa-puffs/documentation/system-snapshot.md
...
const OUTPUT_DIR = join(homedir(), '.hexa-puffs', 'documentation');
const OUTPUT_PATH = join(OUTPUT_DIR, 'system-snapshot.md');
const AGENTS_PATH = join(process.cwd(), 'agents.json');

// Read auth token
let HEXA_PUFFS_TOKEN = process.env.HEXA_PUFFS_TOKEN || '';
if (!HEXA_PUFFS_TOKEN) {
  try {
    HEXA_PUFFS_TOKEN = readFileSync(join(homedir(), '.hexa-puffs', 'hexa-puffs.token'), 'utf-8').trim();
  } catch {
...
async function fetchOrchestrator(path: string): Promise<unknown> {
  ...
  if (HEXA_PUFFS_TOKEN) {
    headers['X-Hexa-Puffs-Token'] = HEXA_PUFFS_TOKEN;
  }
