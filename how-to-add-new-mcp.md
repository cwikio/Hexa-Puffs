# Hexa Puffs MCP Guide

This document covers how to add MCPs (Model Context Protocol servers) to Hexa Puffs, both external third-party MCPs and custom internal MCPs.

## Table of Contents

### Adding External MCPs (Quick Start)
- [External MCPs Overview](#external-mcps)
- [Configuration File](#configuration-file-external-mcpsjson-project-root)
...
External MCPs are third-party MCP servers (like Vercel, PostHog, Neon) that run outside the Hexa Puffs monorepo. They are the easiest way to add new capabilities to Hexa Puffs.

### Configuration File: `external-mcps.json` (project root)
...
- Detects tools where the name contains `delete`, `remove`, or `destroy` as a standalone word or underscore-separated segment (case-insensitive, regex: `/^(.*_)?(delete|remove|destroy)(_.*)?$/i`)
...
1. Edit `external-mcps.json` in the project root — add a new entry with command/args/env
...
The following sections describe how to create custom internal MCPs that live in the Hexa Puffs monorepo. This is an advanced topic — most users should start with external MCPs above.

### Quick Start Checklist

1. Create a directory in the MCPs root (e.g. `MyTool-MCP/`)
2. Add a `package.json` with `"type": "module"` and an `"hexa-puffs"` manifest field
...
│   ├── package.json          # Must include "hexa-puffs" manifest
...
The Orchestrator's scanner reads every sibling directory's `package.json` looking for an `"hexa-puffs"` field. If present and valid, the MCP is registered automatically.
...
> **Note:** `allowDestructiveTools` is not part of `HexaPuffsManifest` — it only applies to external MCPs (configured in `external-mcps.json` via the `metadata` field). Internal MCPs always expose all their tools.
...
2. Skips directories without a `package.json` or without an `"hexa-puffs"` field
...
| `@mcp/shared/Utils/config.js` | `getEnvString()`, `getEnvNumber()`, `getEnvFloat()`, `getEnvBoolean()`, `requireEnvString()`, `expandPath()` |
| `@mcp/shared/Utils/env.js` | `loadEnvSafely()` — safe `.env` loader that prevents dotenv v17 stdout corruption |
...
export function createServer(): McpServer {
  const server = new McpServer({
    name: "mytool",   // Must match mcpName in package.json hexa-puffs field
    version: "1.0.0",
  });
...
`loadEnvSafely()` resolves the `.env` file relative to the package root, guards with `existsSync()`, and passes `quiet: true` to prevent dotenv v17 from writing debug output to stdout.
