...
│  + Date/time + Chat ID (if available) + "## Current Task" header    │
│  + relevant memories                                                 │
│  NO: playbooks, available skills, compaction summary                 │
...
| `Thinker/prompts/fact-extraction-prompt.md` | Fact extraction prompt template (`{{KNOWN_FACTS}}`, `{{USER_IDENTITY}}`, `{{CONVERSATION}}` placeholders) |
...
| `Thinker/src/agent/skill-loader.ts` | Parses `~/.hexa-puffs/skills/*/SKILL.md` |
...
Priority 1 ─── Custom file prompt (THINKER_SYSTEM_PROMPT_PATH env var)
    │               Written by AgentManager to ~/.hexa-puffs/agent-prompts/{agentId}.txt
    │               Set when agent config has a non-empty systemPrompt field
    │
    ├── If not set ──▶ Priority 2 ─── Persona file
    │                       Loaded from ~/.hexa-puffs/agents/{agentId}/instructions.md
...
│   "agentId":     │          │                          │       │                  │
│     "hexa-puffs", │  load    │  1. Write systemPrompt   │ env   │  Read file at    │
│   "systemPrompt":│ ──────▶ │     to temp file:         │ var   │  THINKER_SYSTEM_ │
│     "You are..." │          │     ~/.hexa-puffs/         │ ────▶│  PROMPT_PATH     │
│ }                │          │     agent-prompts/        │       │                  │
└─────────────────┘          │     hexa-puffs.txt         │       │  Store as        │
                              │                          │       │  this.custom-    │
...
`buildContext()` (`loop.ts:503-608`) appends six dynamic layers onto the base prompt. Each layer is simple string concatenation. The order matters — later layers appear at the end of the system prompt.

### Layer 1: Date/Time (lines 530-543)
...
### Layer 3: Compaction Summary (lines 549-551)
...
### Layer 4: Matched Playbooks (lines 554-565)

```typescript
// Refresh cached playbooks (5-min TTL for DB, file-based re-scanned on each refresh)
...
### Layer 5: Available Skills — Progressive Disclosure (lines 568-577)

File-based skills (`~/.hexa-puffs/skills/*/SKILL.md`) that have **no keywords** are injected as XML descriptions. This tells the LLM what capabilities exist without injecting full instructions:
...
Skills **with keywords** participate in playbook classification (Layer 4) and inject their full instructions when matched.

### Layer 6: Relevant Memories (lines 580-584)
...
`processProactiveTask()` (`loop.ts:1176-1439`) builds a simpler prompt for scheduled/cron skill executions.
...
| Chat ID | Yes — injected | **Yes** — injected when `chatId` is provided |
| Compaction summary | Yes — if available | **No** |
| `## Current Task` header | No | **Yes** — appended with instructions |
| Conversation history | Last 30 messages (repaired, truncated) | **None** — task instructions as sole user message |
...
The `toolChoice` parameter defaults to `'auto'`. Two guards still use `'required'` in specific recovery scenarios:

1. **Hallucination guard** — When the model claims it performed an action but called no tools, a retry fires with `toolChoice: 'required'` and `maxSteps: 8` to force actual tool execution.
2. **Tool-refusal guard** — When the model says "I don't have access to real-time data" despite having search tools, a single forced call fires with `toolChoice: 'required'` and `maxSteps: 1` (limited to 1 step to avoid a Groq/Llama crash where `'required'` on step 2+ fails).
...
|-------|------|----------------------|
| `agentId` | string | Determines persona file path (`~/.hexa-puffs/agents/{agentId}/instructions.md`) |
...
1. **Database playbooks** (Memorizer MCP) — 16 seeded defaults covering email, calendar, search, memory, Vercel, skills, etc. Stored in the `skills` table with `trigger_type: 'event'`.

2. **File-based skills** (`~/.hexa-puffs/skills/*/SKILL.md`) — follow the [agentskills.io](https://agentskills.io/specification) specification with Hexa Puffs extensions in the `metadata` block.
...
- **File-based skills**: Scanned at startup and re-scanned on every 5-minute refresh cycle (picks up new/changed SKILL.md files)
...
5. Builds a structured extraction prompt by substituting `{{KNOWN_FACTS}}`, `{{USER_IDENTITY}}`, and `{{CONVERSATION}}` placeholders in the template:
...
- Prompt template is file-based (`Thinker/prompts/fact-extraction-prompt.md`) with `{{KNOWN_FACTS}}` / `{{USER_IDENTITY}}` / `{{CONVERSATION}}` placeholders
...
| --------- | ---- | ------------------- |
| Default system prompt (fallback) | `Thinker/src/agent/loop.ts:109-111` | `DEFAULT_SYSTEM_PROMPT` constant (3-line fallback) |
| Default system prompt (file) | `Thinker/prompts/default-system-prompt.md` | File-based prompt loaded at startup |
| Prompt loading (init) | `Thinker/src/agent/loop.ts:194-303` | `initialize()` — loads custom + persona + default prompts |
| Context building | `Thinker/src/agent/loop.ts:503-608` | `buildContext()` — all 6 dynamic layers + profile persona override |
| Proactive prompt | `Thinker/src/agent/loop.ts:1176-1439` | `processProactiveTask()` — skill prompt assembly + profile persona override |
| Playbook injection | `Thinker/src/agent/loop.ts:553-565` | Force-inject playbook required tools (with missing-tool warning) |
...
| Playbook seeds | `Thinker/src/agent/playbook-seed.ts` | 16 default playbook definitions |
...
| Fact extraction | `Thinker/src/agent/fact-extractor.ts:48-61,145-189` | `loadExtractionPromptTemplate()`, `extractFactsFromConversation()` |
| Fact extraction prompt (file) | `Thinker/prompts/fact-extraction-prompt.md` | File-based template with `{{KNOWN_FACTS}}`/`{{USER_IDENTITY}}`/`{{CONVERSATION}}` placeholders |
