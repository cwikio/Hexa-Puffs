...
- `GRANTS_DB_PATH` — Grant storage (default: `~/.hexa-puffs/data/grants.db`)
...
- Forbidden: `~/.ssh/`, `~/.gnupg/`, `~/.aws/`, `~/.config/`, `/etc/`, `/var/`, `~/.hexa-puffs/data/`
- System grants auto-created for `~/.hexa-puffs/documentation/` and `~/.hexa-puffs/logs/`
...
**Purpose:** Standalone bridge MCP that lets external AI assistants (Claude Desktop, Claude Code, etc.) talk to the Hexa Puffs Orchestrator over HTTP.
**Transport:** Stdio (to host) + HTTP (to Orchestrator) | **Auto-discovered:** No — not part of the core system

**Tools:**

- `chat` — Send a message to Hexa Puffs and get a response (POST `/chat`, 60s timeout)
- `check_status` — Get system status (GET `/status`)
- `check_notifications` — Check for async notifications (placeholder)

**Resources:**

- `hexa-puffs://notifications` — Recent async events (placeholder)
- `hexa-puffs://logs` — Real-time system logs (placeholder)

**Configuration:**

- `ORCHESTRATOR_URL` — Orchestrator address (default: `http://localhost:3000`)
- `HEXA_PUFFS_TOKEN` — Auth token (falls back to `~/.hexa-puffs/hexa-puffs.token`)

**Note:** This MCP is installed in the host's MCP client config (e.g., Claude Desktop's `claude_desktop_config.json`), not managed by the Orchestrator. It does not have a `"hexa-puffs"` manifest in `package.json`.
...
| File | Purpose |
|------|---------|
| Each MCP's `package.json` | `"hexa-puffs"` manifest (mcpName, transport, sensitive, role) |
...
| `Connector-MCP/src/index.ts` | Standalone bridge MCP for external AI assistants |
