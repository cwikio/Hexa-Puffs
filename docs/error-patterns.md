...
1. **MCP is down** — The MCP that provides the tool crashed or failed to start.
   ...
   - Fix: MCP should auto-restart within 60 seconds (health check interval). If stuck, check `~/.hexa-puffs/logs/orchestrator.log` for spawn errors.

2. **Tool name prefix mismatch** — Tool names are prefixed with MCP name (e.g., `memory_store_fact`). LLM may hallucinate wrong prefix.
   - Check: `/help` — shows all available tools with correct names.
...
**Symptoms:** Messages are dropped, Hexa Puffs doesn't respond. `/status` shows agent as "paused."
...
1. **Cost controls triggered** — Token consumption exceeded threshold.
   ...
   - Check: `/status` — shows pause reason. Also check `~/.hexa-puffs/logs/orchestrator.log` (grep for `[thinker:hexa-puffs]`) for cost monitor messages.
...
3. **LLM provider rate limit** — Groq or other provider returned rate limit error.
   - Check: `~/.hexa-puffs/logs/orchestrator.log` (grep for `[thinker:hexa-puffs]`) for rate limit errors.
...
**Symptoms:** Hexa Puffs takes a long time to respond (>30 seconds for simple queries).

**Possible causes:**

1. **LLM provider latency** — Groq, LM Studio, or Ollama is slow.
   - Check: `~/.hexa-puffs/logs/orchestrator.log` (grep for `[thinker:hexa-puffs]`) for LLM call durations.
...
1. **Missing dependency** — MCP needs an external service that isn't available (e.g., Ollama for Memorizer embeddings, `op` CLI for 1Password).
   - Check: `~/.hexa-puffs/logs/orchestrator.log` for MCP stderr output.
...
2. **Gmail MCP down** — MCP failed to start or crashed.
   ...
   - Fix: Check `~/.hexa-puffs/logs/orchestrator.log` for Gmail spawn errors.
...
**Symptoms:** Hexa Puffs calls the wrong tool for a request, or can't find a tool that should be relevant.
...
2. **Embedding cache stale** — Tool list changed but embedding cache hasn't been regenerated.
   - Check: Embedding cache at `~/.hexa-puffs/data/embedding-cache.json` — check `metadata.toolCount`.
...
5. **Read specific logs:** Use `filer_read_file` with path `~/.hexa-puffs/logs/<service>.log` for deeper inspection.
6. **Read documentation:** Use `filer_read_file` with path `~/.hexa-puffs/documentation/<topic>.md` for architecture understanding.
