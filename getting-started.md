...
This guide walks you through setting up Hexa Puffs from a fresh clone.
...
Hexa Puffs integrates with several external services. You don't need all of them to get started — see [Minimal Setup](#minimal-setup) below.
...
3. The token is saved at `~/.hexa-puffs/gmail/token.json`
...
Logs are written to `~/.hexa-puffs/logs/`.
...
```bash
ls ~/.hexa-puffs/logs/
cat ~/.hexa-puffs/logs/orchestrator.log
```
