...
describe("Searcher MCP Tests", () => {
  ...
  describe("1. Health & Initialization", () => {
    it("1.1 should respond to health check with 200 OK", async () => {
      logSection("Searcher MCP Tests (Stdio)");
    ...
    it("1.2 should return correct health data structure", async () => {
      ...
      expect(healthData?.transport).toBe("stdio");
  ...
  describe("10. Error Handling", () => {
    it("10.1 should return error for unknown tool", async () => {
      logSection("Error Handling Tests");
      const result = await callTool(unknownToolName, { query: "test" });
      expect(result.success).toBe(false);
      expect(result.error).toMatch(/unknown|not found|Method not found/i);
    });
    ...
    // Tests 10.3-10.5 removed: they tested HTTP transport behavior
    // (malformed JSON, 404 endpoints, CORS) which don't apply to Stdio transport.
  });
