...
```bash
docker build -t hexa-puffs-sandbox -f Dockerfile.sandbox .
```
...
CODEXEC_DOCKER_IMAGE=hexa-puffs-sandbox             # Docker image name
CODEXEC_SANDBOX_DIR=~/.hexa-puffs/codexec/sandbox   # Working directory for executions
...
# Logging
CODEXEC_LOG_DIR=~/.hexa-puffs/codexec/logs          # Execution logs directory
...
# Scripts
CODEXEC_SCRIPTS_DIR=~/.hexa-puffs/scripts           # Saved scripts directory
...
// Output
{
  ...
  "name": "parse-excel-report",
  "path": "~/.hexa-puffs/scripts/parse-excel-report/script.py",
  "saved_at": "2026-02-09T15:00:00Z"
}
```

**Storage format:** Each script is a directory under `~/.hexa-puffs/scripts/`:
...
Every execution produces a structured JSONL entry at `~/.hexa-puffs/codexec/logs/`.
...
```json
{
  ...
  "memory_peak_mb": 45.2,
  "sandbox_mode": "subprocess",
  "working_dir": "~/.hexa-puffs/codexec/sandbox/exec_a1b2c3",
  "artifacts": {
...
```json
{"type":"session_start","session_id":"sess_x1y2z3","language":"python","name":"data-analysis","pid":12345,"agent_id":"hexa-puffs","started_at":"2026-02-09T14:30:00Z"}
...
**Isolation measures:**
- **Stripped environment:** The spawned process inherits ONLY `PATH`, `HOME`, `LANG`, `TERM`. All API keys, tokens, and Hexa Puffs-internal env vars are removed. The agent's LLM provider keys, Telegram session, Gmail OAuth tokens, 1Password credentials — none are accessible to generated code.
...
- Reading sensitive files (e.g., `~/.ssh/`, `~/.hexa-puffs/data/`)
...
- Credential access (`~/.ssh`, `~/.aws`, `~/.hexa-puffs/data`, environment variable enumeration)
...
- `~/.aws/`
- `~/.config/`
- `~/.hexa-puffs/data/` (internal databases)
- `/etc/`, `/var/`
...
- [ ] `save_script` — persist to `~/.hexa-puffs/scripts/`
